...
// HCons(head=1000, tail=HCons(head=900, tail=arrow.generic.HNil@e3ee751))
```

```kotlin
Account(1000, 900).toHListLabeled()
// HCons(head=Tuple2(a=balance, b=1000), tail=HCons(head=Tuple2(a=available, b=900), tail=arrow.generic.HNil@e3ee751))
```
...
// DeferredK(deferred=LazyDeferredCoroutine{New}@6a3e1cd2)
```
